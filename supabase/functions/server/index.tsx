...
async function initializeData() {
  try {
    ...
    if (newsletters.length === 0) {
      // Create default newsletters (starting from March 2026)
...
app.post("/make-server-ccd31d74/admin/reset-newsletters", async (c) => {
  try {
    ...
    // Create only March 2026 newsletter
    ...
    await kv.set(`newsletter:${marchNewsletter.id}`, marchNewsletter);

    return c.json({ success: true, message: "Newsletters reset to March 2026 only" });
  } catch (error) {
